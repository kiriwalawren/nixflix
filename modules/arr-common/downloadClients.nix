      ...
      ${concatMapStringsSep "\n" (clientConfig: let
        ...
        in ''
          ...
          ${
            if apiKey == null
            then "CLIENT_API_KEY=''"
            else secrets.toShellValue "CLIENT_API_KEY" apiKey
          }
          ${
            if username == null
            then "CLIENT_USERNAME=''"
            else secrets.toShellValue "CLIENT_USERNAME" username
          }
          ${
            if password == null
            then "CLIENT_PASSWORD=''"
            else secrets.toShellValue "CLIENT_PASSWORD" password
          }
          ...
          if [ -n "$EXISTING_CLIENT" ]; then
            ...
            UPDATED_CLIENT=$(apply_field_overrides "$EXISTING_CLIENT" "$CLIENT_API_KEY" "$CLIENT_USERNAME" "$CLIENT_PASSWORD" "$FIELD_OVERRIDES")
            echo "$UPDATED_CLIENT" > "$TEMP_FILE"
            ...
            NEW_CLIENT=$(apply_field_overrides "$SCHEMA" "$CLIENT_API_KEY" "$CLIENT_USERNAME" "$CLIENT_PASSWORD" "$FIELD_OVERRIDES")
            echo "$NEW_CLIENT" > "$TEMP_FILE"
