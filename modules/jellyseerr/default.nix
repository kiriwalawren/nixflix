...
in {
  ...
  config = mkIf (nixflix.enable && cfg.enable) {
    ...
    in [
      ...
      {
        assertion = cfg.jellyfin.enable -> nixflix.jellyfin.enable;
        message = "Jellyfin must be enabled to use Jellyseerr Jellyfin integration.";
      }
      {
        assertion = cfg.jellyfin.enable -> (any (user: user.policy.isAdministrator) (attrValues nixflix.jellyfin.users));
        message = "At least one Jellyfin admin user must be configured for Jellyseerr integration.";
      }
      {
