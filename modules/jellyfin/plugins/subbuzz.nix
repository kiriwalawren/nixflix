...
in
{
  options.nixflix.jellyfin.plugins.subbuzz = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = ''
        Whether or not to enable the [SubBuzz](https://github.com/josdion/subbuzz) plugin.
        SubBuzz is a subtitle downloader that supports several subtitle providers.
      '';
    };

    package = lib.mkOption {
      inherit (jellyfinPlugins.packageModule) type description example;
      default = jellyfinPlugins.fromRepo {
        version = "1.4.1.0";
