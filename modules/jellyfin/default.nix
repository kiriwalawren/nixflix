      ...
      serviceConfig =
        {
          ...
          ExecStartPost =
            "+"
            + pkgs.writeShellScript "jellyfin-wait-ready" ''
              set -eu
              ...
              for i in {1..240}; do
                ...
                if [ "$HTTP_CODE" = "200" ]; then
                  ...
                  STARTUP_CODE=$(${pkgs.curl}/bin/curl --connect-timeout 5 --max-time 10 -H "$AUTH_HEADER" -s -o /dev/null -w "%{http_code}" -X GET "$BASE_URL/Startup/Configuration" 2>/dev/null || echo "000")
